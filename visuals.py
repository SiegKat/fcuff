...
def plot_cv(data=None, use_all=False, fig=None, ax=None, labels=None, line=True, scatter=False, errs=False, current_column=1, potential_column=0, err_column=3, xunits='V', yunits=r'$mA/cm^2$', export_name=None, export_type='png', fig_kw={}, err_kw={}, **plot_kw):
	...
	for d in data:
		...
		x = datums.find_col(this_data, 'Potencial', potential_column)
		y = datums.find_col(this_data, 'Corrente', current_column)
		yerr = check_errs(errs, this_data, 'Erro_Corrente', err_column)
		lines, caps, bars = plotter(ax, x, y, yerr, this_label, line, scatter, errs, err_kw, **plot_kw)
		d.set_line(lines)
		d.set_errcaps(caps)
		d.set_errbars(bars)
	if len(data) > 1:
		ax.legend(loc='Melhor', edgecolor='k')
	ax.set_xlabel(build_axlabel('Potencial', xunits))
	ax.set_ylabel(build_axlabel('Densidade de Corrente', yunits))
	if export_name:
...
def polcurve(data=None, use_all=False, fig=None, ax=None, labels=None, line=True, scatter=True, errs=False, current_column=0, potential_column=1, err_column=3, xunits=r'$mA/cm^2$', yunits='V', export_name=None, export_type='png', fig_kw={}, err_kw={}, **plot_kw):
	...
	for d in data:
		...
		this_data.columns = utils.check_labels(this_data)
		this_label = d.get_label()
		x = datums.find_col(this_data, 'Corrente', current_column)
		y = datums.find_col(this_data, 'Potencial', potential_column)
		yerr = check_errs(errs, this_data, 'Erro_Potencial', err_column)
		lines, caps, bars = plotter(ax, x, y, yerr, this_label, line, scatter, errs, err_kw, **plot_kw)
		d.set_line(lines)
		d.set_errcaps(caps)
		d.set_errbars(bars)
	if len(data) > 1:
		ax.legend(loc='Melhor', edgecolor='k')
	ax.set_xlabel(build_axlabel('Densidade de Corrente', xunits))
	ax.set_ylabel(build_axlabel('Potencial', yunits))
	if export_name:
		fig_saver(export_name, export_type)
	return fig, ax
...
def plot_cp_raw(data=None, use_all=False, fig=None, ax=None, labels=None, line=False, scatter=True, errs=False, current_column=2, potential_column=1, time_column=0, err_column=(4,5), xunits='s', yunits=('V', 'mA'), export_name=None, export_type='png', fig_kw={}, err_kw={}, **plot_kw):
	...
	for d in data:
		...
		this_data.columns = utils.check_labels(this_data)
		this_label = d.get_label()
		x = datums.find_col(this_data, 'Tempo', time_column)
		y1 = datums.find_col(this_data, 'Potencial', potential_column)
		y2 = datums.find_col(this_data, 'Corrente', current_column)
		yerr1 = check_errs(errs, this_data, 'Erro_Potencial', err_column[0])
		yerr2 = check_errs(errs, this_data, 'Erro_Corrente', err_column[1])
	...
	ax.set_xlabel(build_axlabel('Tempo', xunits))
	ax.set_ylabel(build_axlabel('Potencial', yunits[0]))
	ax2.set_ylabel(build_axlabel('Corrente', yunits[1]))
...
def plot_tafel(data=None, use_all=False, fig=None, ax=None, labels=None, line=False, scatter=True, errs=False, current_column=3, potential_column=2, err_column=3, xunits='', yunits='V', plot_slope=True, imin=None, imax=None, export_name=None, export_type='png', fig_kw={}, **plot_kw):
	...
	for d in data:
		...
		this_data.columns = utils.check_labels(this_data)
		this_label = d.get_label()	
		x = datums.find_col(this_data, 'Corrente de Taffel', current_column)
		y = datums.find_col(this_data, 'Sobrepotencial', potential_column)
	...
	if len(data) > 1:
		ax.legend(loc='Melhor', edgecolor='k')
	ax.set_xlabel('log(Corrente)')
	ax.set_ylabel(build_axlabel('Sobrepotencial', yunits))
	if export_name:
		fig_saver(export_name, export_type)
	return fig, ax
...
def plot_lsv(data=None, use_all=False, fig=None, ax=None, labels=None, line=False, scatter=True, errs=False, current_column=1, potential_column=2, err_column=3, xunits='V', yunits=r'$mA/cm^2$', export_name=None, export_type='png', fig_kw={}, **plot_kw):
	...
	for d in data:
		...
		this_data.columns = utils.check_labels(this_data)
		this_label = d.get_label()	
		x = datums.find_col(this_data, 'Sobrepotencial', potential_column)
		y = datums.find_col(this_data, 'Corrente', current_column)
	...
	if len(data) > 1:
		ax.legend(loc='best', edgecolor='k')
	ax.set_xlabel(build_axlabel('Sobrepotencial', xunits))
	ax.set_ylabel(build_axlabel('Densindade de Corrente', yunits))
	if export_name:
		fig_saver(export_name, export_type)
	return fig, ax
...
def check_errs(errs, df, err_name, err_col):
	...
	if errs:
		try:
			...
			err = np.zeros(count)
			_log.warning('Não foi possível utilizar os valores fornecidos para o erro.')
	else:
		err = np.zeros(count)
	return err
