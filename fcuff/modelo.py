...
class Datum():
	def __init__(self, name, data):
		...
		self.raw_data = data
		self.processed_data = self.expt_type = None
		
		# processed values
		self.current_data = self.potential_data = self.overpotential_data = self.logcurrent_data = self.realcurrent_data = self.imagcurrent_data = self.error_data = None
			
		# misc parameters
		self.area = 1
		self.refelec = self.thermo_potential = 0
		
		# tafel
		self.tafel_slope = self.exchg_curr = self.tafel_rsq = None
				
		# eis
		self.semicircle_params = self.linearfit_params = self.hfr = self.hfr_linear = self.lfr = self.eis_current = None	

		# visualization
		self.line = self.errcaps = self.errbars = None
